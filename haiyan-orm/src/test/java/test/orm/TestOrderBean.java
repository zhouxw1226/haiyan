...
import test.orm.sql.TestSQLDBM;

public class TestOrderBean {

	public static void main(String[] args) throws Throwable {
		TestSQLDBM t = new TestSQLDBM();
		t.test1();
		System.exit(0);
	}
	@Test
...
//			ConfigUtil.setDataCache(new EHDataCache()); // 配置用缓存框架
		...
		}
		{
			...
			file = new File(TestSQLDBM.class.getResource("SYS.xml").getPath());
			ConfigUtil.loadTableConfig(file, true);
			file = new File(TestSQLDBM.class.getResource("SYSORGA.xml").getPath());
			ConfigUtil.loadTableConfig(file, true);
			file = new File(TestSQLDBM.class.getResource("SYSOPERATOR.xml").getPath());
			ConfigUtil.loadTableConfig(file, true);
			file = new File(TestSQLDBM.class.getResource("TEST_DBM.xml").getPath());
			ConfigUtil.loadTableConfig(file, true);
		}
