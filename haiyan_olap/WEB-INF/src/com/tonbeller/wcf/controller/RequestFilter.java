...
/**
 * Informs <code>RequestListeners</code> of incoming request. If a nextView
 * attribute is present in the current request, it will forward to that.
 * Otherwise the filter chain is executed.
 */
...
public class RequestFilter implements Filter {
	...
	/**
	 * if the session attribute FORCE_INDEX_JSP exists, then the client will be
	 * redirected to index.jsp
	 */
	public static final String FORCE_INDEX_JSP = "com.tonbeller.wcf.controller.FORCE_INDEX_JSP";

	/**
	 * If this request parameter is present, the DomUtils.randomId() will be
	 * reset once.
	 */
	...
	class MyHandler implements RequestSynchronizer.Handler {
		...
		public void normalRequest() throws Exception {
			...
			try {
				...
				if (response.containsHeader("Location"))
					return;
				// someone has called sendError() or sendRedirect() on the
				// response?
		...
		private void handleException(Exception e) throws Exception {
			...
			if (errorJSP != null) {
				try {
					...
					request.getRequestDispatcher(errorJSP).forward(request,
							response);
				} catch (Exception e2) {
		...
		public void showBusyPage(boolean redirect) throws Exception {
			...
			if (busyJSP != null) {
				...
				throw new IllegalStateException(
						"concurrent requests and no busy.jsp defined in web.xml");
		}
		...
		/**
		 * true, if the current request was redirected to the index page because
	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		MultiPartEnabledRequest request = new MultiPartEnabledRequest(
		...
		try {
			...
			if (logger.isInfoEnabled())
				logger.info("Request Execution total time: " + (t2 - t1)
						+ " ms");
			//
		} catch (Throwable e) {
			...
			try {
				...
				logger
						.error("No output writer could be retrieved, logging to stdout");
			}
