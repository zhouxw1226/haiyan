 ...
 * 
 */
public class TestTornadoMondrian {
	...
	public static void main(String[] args) throws Exception {
		String connString = "Provider=mondrian;"
			...
			+ "Catalog=" + FileUtil.path + "/Tornado.xml;";
		String queryString = ""
			// + "{TopCount([USER].[USER_DEPT].members, 5, [Measures].[CODE])} on rows "
			// + " {[USER].members} on rows "
			+ " SELECT "
			+ " {[Measures].[合计客户],[Measures].[有效客户],[Measures].[无效客户]} on columns, "
			+ " {([人员].[所有用户],[省份].[所有省份])} on rows "
			+ " FROM [CUSTOMER] ";
		// + " WHERE [城市].[城市名称].[苏州市] ";
		// .members
		String html = new TestTornadoMondrian().print(connString, queryString).toString();
		FileUtil.String2File(html, "F:/testTornadoMondrian.html", false);
	}
